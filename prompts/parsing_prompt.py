...
🔹 **일반적인 오류 패턴:**
- 객체 배열 구조 오류: `{"position": "AI"}, {"task": "개발"}`
- 키 이름 오타: `postion`, `sub_tasks`
- 값 타입 오류: `"subtasks": "작업1, 작업2"`
- 불완전한 구조: `[{"position": "AI"`
...
🔹 **예시:**
입력: `[{position: AI, task: 모델개발, subtasks: [데이터수집, 모델훈련]}]`
출력: `[{"position": "AI", "task": "모델개발", "subtasks": ["데이터수집", "모델훈련"]}]`"""),
...
수정된 JSON:""")
    ...
    def get_general_json_prompt(self, raw_text: str, position: str, tasks: str) -> List:
        ...
        """
        return [
            SystemMessage(content=f"""당신은 JSON 형식 오류를 수정하는 전문가입니다.

🔹 **수정 규칙:**
1. **구조 복원**: 누락된 구문 요소 추가
2. **문법 수정**: 올바른 JSON 문법으로 변환
3. **데이터 정리**: 불필요한 요소 제거
4. **형식 통일**: 일관된 JSON 구조 유지
